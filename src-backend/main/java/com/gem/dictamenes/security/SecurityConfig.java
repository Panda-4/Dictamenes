...
public class SecurityConfig {
    ...
    @Value("${FRONTEND_URL:http://localhost:3000,http://localhost:5173}")
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                // Crear/editar solicitudes: solo roles con escritura
                .requestMatchers(HttpMethod.POST, "/api/solicitudes/**").hasAnyRole("ADMINISTRADOR", "AUTORIZADOR", "CAPTURISTA")
                .requestMatchers(HttpMethod.PUT, "/api/solicitudes/**").hasAnyRole("ADMINISTRADOR", "AUTORIZADOR", "CAPTURISTA")
                // Auditoría: solo ADMINISTRADOR
                .requestMatchers("/api/auditoria/**").hasRole("ADMINISTRADOR")
                // Gestión de usuarios: solo ADMINISTRADOR
                .requestMatchers("/api/usuarios/**").hasRole("ADMINISTRADOR")
                // Importación: solo ADMINISTRADOR
                .requestMatchers("/api/solicitudes/import/**").hasRole("ADMINISTRADOR")
            ...
            .headers(headers -> {
                headers.addHeaderWriter(new StaticHeadersWriter("Server", "Webserver"));
                headers.contentSecurityPolicy(csp -> csp.policyDirectives(
                    ...
                    "connect-src 'self' http://localhost:8080 http://localhost:5173 https://*.gem.gob.mx;"
                ));
